  ...
  return async dispatch => {
    if (!city.length) {
      ...
      try {
        ...
        const weatherDays = await axios.get(`http://api.openweathermap.org/data/2.5/forecast?q=${cityState[0]}\,,${cityState[1]}&appid=6d5233c17d482d1c20dabfc48d8b3112&units=imperial`);
        
        const newData = weatherDays.data.list.filter(({dt_txt}) => {
